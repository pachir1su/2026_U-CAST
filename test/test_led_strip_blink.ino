/* ============================================================================
   ...
   **가진 부품만으로 조립하고 동작을 확인**하려고 만든 축소판입니다.
   ...
   [센서를 4개로 늘릴 때]
   이 테스트판은 D8·D9 두 센서 전용입니다.
   최종 4센서 구성으로 확장할 때는 저장소 루트 `main.ino`의 핀 배정표에 맞춰
   별도로 재배선하세요. 테스트판의 핀 배정을 그대로 확장 기준으로 사용하지 않습니다.
   ...
   어느 쪽에서 출발하든 똑같이 동작합니다.
     ...
     [대기]  오른쪽(D9)이 먼저 감지  →  [경고 중]  (도착 쪽 = 왼쪽으로 기억)

     [경고 중]  기억해 둔 반대쪽이 감지 →  [대기]
     [경고 중]  출발한 쪽이 다시 감지   →  경고 유지

   경고 중에는 스트립(D4)과 빨간 LED(D3)가 0.3초 간격으로 함께 점멸하며,
...
// 이 테스트판은 D8·D9의 IR 센서 2개만 사용합니다.
// 최종 4센서 구성은 `main.ino`의 핀 배정표를 따르세요.
...
/* ---------------------------------------------------------------------------
   [설정 4] 경고 출력 주기
   --------------------------------------------------------------------------- */
// 아래 점멸 간격은 **네오픽셀 스트립과 빨간 경고 LED**에 함께 쓰입니다. 부저는 사이렌 설정을 따릅니다.
...
bool warningBlinkOn = false;         // 지금 스트립과 빨간 경고 LED가 켜진 상태인지
...
/* 스트립과 빨간 경고 LED의 점멸을 담당합니다.
   delay()를 쓰면 기다리는 동안 센서와 버튼을 못 읽으므로,
   "마지막으로 뒤집은 뒤 WARNING_BLINK_MS가 지났는가?"만 확인하고 빠져나옵니다.
   부저는 끊지 않고 별도의 사이렌 로직으로 계속 울립니다. */
void updateBlinkOutputs(uint32_t now) {
  if ((now - warningBlinkChangedAt) < WARNING_BLINK_MS) return;
  warningBlinkChangedAt = now;

  bool nextOn = !warningBlinkOn;  // 켜짐↔꺼짐
  applyBlinkOutputs(nextOn);
  applyStrip(nextOn);
}
...
void logIrValues(uint32_t now) {
  ...
  Serial.print((systemState == STATE_WARNING && warningBlinkOn)
                   ? F(" 스트립=빨강")
                   : F(" 스트립=꺼짐"));
...
void setup() {
  ...
  if (SERIAL_DEBUG) {
    ...
    Serial.println(F("왼쪽 IR=D8, 오른쪽 IR=D9, LED=D3, 스트립=D4, 부저=D5"));
    if (!USE_BUTTONS) {
...
void loop() {
  ...
  switch (systemState) {
    ...
    case STATE_WARNING: {
      ...
      // 여기까지 왔으면 경고 유지 중 → 스트립·LED 점멸과 부저 사이렌을 계속 돌립니다.
