...
아두이노 장치입니다. 경고 중에는 네오픽셀 스트립 2줄이 전체 빨간색으로 켜지고,
...
| LED 스트립 1 DIN | D6 | `PIN_STRIP_1` | 경고 조명 1 |
| LED 스트립 2 DIN | D7 | `PIN_STRIP_2` | 경고 조명 2 |
...
| `applyStrips()` | 두 스트립을 전체 빨간색으로 켜거나 완전히 끔 |
...
| 상수 | 기본값 | 의미 |
|---|:--:|---|
| `STRIP1_PIXELS`, `STRIP2_PIXELS` | 15 | **아직 실물로 확정하지 않은 임시값.** 각 스트립의 실제 LED 개수로 수정 |
...
- `tests/test_state_machine.cpp` — 시작·종료·무효 입력·입력 안정성·출력 검증 22개
...
4. 진입 IR 2개(D2·D3)를 설치합니다.
5. 통과 IR 2개(D4·D5)를 설치합니다.
6. 진입 IR → 경고 시작, 통과 IR → 경고 종료 순서로 자동 시연을 확인합니다.

### 발표 때 보여줄 핵심 장면

- 진입 IR 감지 → 스트립 2줄 빨간색 점등 + 빨간 LED·부저 점멸
