...
차도 진입 구간의 **진입 감지 IR 센서 2개**로 보행자의 차도 진입을 감지해 경고를 시작하고,
도로 반대편의 **통과 감지 IR 센서 2개**로 보행자가 다 건넌 것을 감지해 경고를 종료하는
아두이노 장치입니다. 경고 중에는 네오픽셀 스트립 1줄이 전체 빨간색으로 켜지고,
운전자 경고용 빨간 LED와 부저가 함께 점멸합니다.

> 이 MVP는 보행자의 횡단 **방향을 계산하지 않습니다.**
> 진입 상황과 통과 완료 상황만 구분해 경고를 시작하고 종료합니다.
...
| 구분 | 내용 |
|---|---|
| 진입 감지 | 차도 진입 구간에 진입 감지 IR 2개를 설치해 보행자의 진입을 감지 |
| 통과 감지 | 도로 반대편 통과 지점에 통과 감지 IR 2개를 설치해 횡단 완료를 감지 |
| 경고 시작 | 진입 감지 시 경고 시작 |
| 경고 종료 | 통과 감지 시 경고 종료 |
| 운전자 경고 | 네오픽셀 스트립 1줄 전체 빨간색 점등 + 운전자 경고용 빨간 LED 점멸 |
...
```text
entryDetected = entrySensor1Detected || entrySensor2Detected
startRequested = entryDetected || redButtonPressed

exitDetected  = exitSensor1Detected || exitSensor2Detected
stopRequested = exitDetected || greenButtonPressed
```
...
| 상태 | 입력 | 처리 |
|---|---|---|
| 대기 | 진입 IR 1·2 동시 감지 | 경고를 한 번만 시작 |
| 대기 | 통과 IR만 감지 | 무시(상태 변화 없음) |
| 대기 | 진입 IR과 통과 IR 동시 감지 | **모호한 입력으로 보고 시작하지 않음.** 시리얼에 `입력 무시: 진입과 통과가 동시에 감지됨`을 남깁니다 |
| 대기 | 초록 버튼 | 무시 |
| 경고 중 | 통과 IR 1·2 동시 감지 | 종료 |
| 경고 중 | 진입 IR 재감지 / 빨간 버튼 재입력 | 경고 유지(상태 변경 없음) |

빨간 버튼은 방향·센서와 무관한 **강제 시작**이므로 모호 입력 판정의 영향을 받지 않습니다.
초록 버튼도 같은 이유로 **강제 종료**로 동작합니다.
...
| 네오픽셀 LED 스트립 | 1 | 경고 중 전체 빨간색 점등 (LED 10개) |
...
| 부품 | 핀 | 코드 상수 | 역할 |
|---|:--:|---|---|
| 진입 감지 IR 센서 1 | D2 | `PIN_IR_ENTRY_1` | 차도 진입 감지 |
| 진입 감지 IR 센서 2 | D3 | `PIN_IR_ENTRY_2` | 차도 진입 감지 |
| 통과 감지 IR 센서 1 | D4 | `PIN_IR_EXIT_1` | 통과(도착) 감지 |
| 통과 감지 IR 센서 2 | D5 | `PIN_IR_EXIT_2` | 통과(도착) 감지 |
| LED 스트립 DIN | D6 | `PIN_STRIP` | 경고 조명 |
| 빨간 경고 LED | D8 | `PIN_WARN_LED` | 운전자 경고용 점멸 |
...
스트립을 1줄로 확정하면서 **D7은 비어 있습니다.** 아무것도 연결하지 않습니다.

### IR 센서 배선

제품마다 선 색과 핀 순서가 다르므로 **제품 설명의 핀 표기를 우선 확인**합니다.

- 각 모듈: `VCC` → 5V, `GND` → GND, `OUT` → D2 / D3 / D4 / D5
- 감지 시 출력이 LOW가 아니면 코드의 `IR_ACTIVE_LOW`를 `false`로 변경합니다.
- 출력이 오픈 컬렉터 방식이면 `IR_USE_INTERNAL_PULLUP = true`를 사용합니다.

### 네오픽셀 배선

- 스트립 `DIN` → D6
- 데이터선에 330~470Ω 직렬 저항 권장
- 스트립 `5V` → 외부 5V 전원
- 스트립 `GND` → 외부 전원 GND
...
- 빨간 LED: D8 → 220~330Ω → LED `+`, LED `−` → GND
- 부저: `+` → D9, `−` → GND
- 빨간 버튼: 한쪽 → A0, 반대쪽 → GND
- 초록 버튼: 한쪽 → A1, 반대쪽 → GND
...
| 상태 | 의미 |
|---|---|
| `STATE_IDLE` | 대기. 모든 출력 꺼짐. 진입 IR 또는 빨간 버튼을 기다림 |
| `STATE_WARNING` | 경고 중. 스트립·빨간 LED·부저 작동. 통과 IR 또는 초록 버튼을 기다림 |
...
| `applyStrip()` | 스트립을 전체 빨간색으로 켜거나 완전히 끔 |
...
| 상수 | 기본값 | 의미 |
|---|:--:|---|
| `STRIP_PIXELS` | 10 | 스트립에 박힌 낱개 LED 개수. **실물 10개로 확정** |
...
|---|---|---|
| 진입 감지 | IR 2개, D2·D3 | **IR 1개, D2** |
| 통과 감지 | IR 2개, D4·D5 | **IR 1개, D3** |
| LED 스트립 | 1줄, D6 | 1줄, D6 |
| 빨간 경고 LED | D8 | D8 |
| 부저 | D9 | D9 |
| 빨간 버튼 | A0 | A0 |
...
IR을 뺀 나머지 핀은 두 파일이 완전히 같습니다. 그래서 **센서 2개를 더 구하면
D3의 통과 IR을 D4로 옮기고 D3·D5에 센서를 추가한 뒤 `main.ino`를 올리기만 하면**
나머지 배선은 그대로 두고 최종 구성이 됩니다.
...
3. 네오픽셀 스트립(D6)을 연결합니다.
4. 지금 가진 IR 2개를 D2(진입)·D3(통과)에 붙이고 `test/test.ino`로 확인합니다.
5. IR 2개를 더 구하면 통과 IR을 D4·D5로 옮겨 4개를 설치하고 `main.ino`를 올립니다.
6. 진입 IR → 경고 시작, 통과 IR → 경고 종료 순서로 자동 시연을 확인합니다.

### 발표 때 보여줄 핵심 장면

- 진입 IR 감지 → 스트립 빨간색 점등 + 빨간 LED·부저 점멸
- 통과 IR 감지 → 모든 출력 종료 후 대기 복귀
...
- 이 MVP는 보행자의 횡단 방향을 계산하지 않습니다.
- 진입 IR과 통과 IR이 동시에 감지되면 모호한 입력으로 보고 경고를 시작하지 않습니다.
- 여러 사람이 동시에 오갈 때 개별 보행자를 구분하지 않습니다.
- 직사광선, 설치 각도, 제품별 출력 논리 차이로 오작동할 수 있습니다.
- 자동 종료가 기본 비활성이므로, 통과 IR이 감지되지 않으면 경고가 계속 유지됩니다.
  이때는 초록 버튼으로 종료합니다.
