...
경고를 시작해 반대편에 도착할 때까지 유지하는 아두이노 장치.
경고 중에는 네오픽셀 스트립이 전체 빨간색으로 켜지고
운전자 경고용 빨간 LED가 점멸하며 부저가 사이렌 소리를 냅니다.
...
| 소리 경고 | 수동 부저가 800Hz↔1800Hz를 오르내리는 사이렌 소리를 냄(상승·하강 각 0.5초) |
...
| 수동(passive) 부저 | 1 | 운전자에게 청각적으로 경고(사이렌). 능동 부저는 음이 고정되어 사이렌이 되지 않음 |
| 빨간 버튼 | 1 | 경고 수동 시작 |
| 초록 버튼 | 1 | 경고 수동 종료 |
...
| `STATE_WARNING` | 경고 중. 스트립 점등, 빨간 LED 점멸, 부저 사이렌. **출발 쪽의 반대편** IR 또는 초록 버튼을 기다림 |
...
| `enterWarning()` | 스트립 전체 점등, 빨간 LED 점멸 시작, 부저 사이렌 시작 |
| `enterIdle()` | 모든 출력을 끄고 대기 상태 복귀 |
| `applyStrips()` | 연결된 스트립을 전체 빨간색으로 켜거나 완전히 끔 |
| `setStripColor()` | 스트립 하나의 모든 픽셀 색을 정하고 한 번에 전송 |
| `applyBlinkOutputs()` | 빨간 경고 LED를 켜거나 끔 |
| `updateBlinkOutputs()` | `WARNING_BLINK_MS` 간격으로 LED 점멸 상태 전환 |
...
| `logIrValues()` | IR 4개의 핀값·판정과 상태·출력을 `IR_LOG_INTERVAL_MS`마다 출력 |
...
| `WARNING_BLINK_MS` | 300 | 빨간 경고 LED 점멸 간격(부저에는 쓰지 않음) |
...
- `delay()`를 쓰지 않으므로 사이렌이 울리는 중에도 IR 센서와 버튼을 계속 읽습니다.
  사이렌 한가운데에서도 반대편 도착·초록 버튼으로 바로 종료됩니다.
- 경고가 끝나면 즉시 `noTone()`으로 소리를 멈춥니다.
...
| 종류 | 시점 | 예 |
|---|---|---|
| 상태 전환·오류 로그 | 발생 즉시 | `상태: 경고 시작 - 왼쪽에서 차도 진입`, `입력 무시: 양쪽이 동시에 감지됨 - 센서 배치를 확인하세요` |
| 주기 진단 로그 | `IR_LOG_INTERVAL_MS`(0.4초)마다 | 아래 형식 |

```text
D8=1 없음  D9=1 없음  D10=0 감지  D11=1 없음  STATE_WARNING SIDE_RIGHT 스트립=빨강 LED=켜짐 부저=1240Hz(상승) 자동복귀=사용안함
```

| 항목 | 의미 |
|---|---|
| `D8=1` | 그 핀의 `digitalRead()` 원시값(0 = LOW, 1 = HIGH) |
| `감지` / `없음` | `IR_ACTIVE_LOW`와 확정 시간까지 적용한 센서별 판정 |
| `STATE_IDLE` / `STATE_WARNING` | 현재 상태 |
| `SIDE_NONE` / `SIDE_LEFT` / `SIDE_RIGHT` | 이번 횡단의 출발 쪽 |
| `스트립=`, `LED=`, `부저=` | 지금 출력 상태(부저는 현재 주파수와 상승/하강) |
| `자동복귀=` | `WARNING_TIMEOUT_MS` 사용 여부와 남은 시간 |
...
- `tests/test_state_machine.cpp` — `main.ino`의 시작·종료·무효 입력·입력 안정성·출력·핀 배정·사이렌·로그 검증 32개
- `tests/test_reduced_sketch.cpp` — `test/test.ino`가 같은 사이렌·로그 동작을 하는지 검증 5개
- 같은 테스트를 `IR_LOG_INTERVAL_MS = 0`으로 한 번 더 빌드해 "로그 끔" 동작까지 확인합니다
...
| `IR_LOG_INTERVAL_MS` | `400` | `main.ino`와 같은 값. 축소판은 IR 2개만 찍습니다 |
...
- 왼쪽에서 진입 → 스트립 빨간색 점등 + 빨간 LED 점멸 + 부저 사이렌 → 오른쪽 도착 → 종료
